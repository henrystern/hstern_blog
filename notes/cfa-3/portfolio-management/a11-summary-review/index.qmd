...
Classifying investors can be useful but is not always completely correct.
...
Econometric models are robust but complex.
...
Liability-relative approaches focus on funding liabilities. Risk focus is principally on ability to pay of liabilities.
...
Synthetic Long = $c_0 + p_0$

Synthetic Short = $-(c_0 + p_0)$
...
Cross currency basis swaps can be used to exchange borrowed domestic currency funds into the required foreign currency while reducing exchange risk.
...
Total return swaps have a lower initial outlay but carry counter party risk.
...
No hedge fund index is all-encompassing.
...
* An IPS incudes the following sections:
