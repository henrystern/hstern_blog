...
While that works fine where the iteration occurs in a single loop, generator functions are much cleaner if the next item is needed at unpredictable intervals, or in multiple locations. Since generators store both the state and value in a single object there is no need to remember to change the state variable each time you call the function. This keeps the code cleaner and reduces the likelihood of making an error. 
